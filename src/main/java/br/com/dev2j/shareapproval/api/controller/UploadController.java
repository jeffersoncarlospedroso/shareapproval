...
public class UploadController {
	 ...
	 * 
	 * @PostMapping public ResponseEntity<Void> save(@RequestBody @Valid final
	 * UserNewDto userNewDto) { User user = userService.createUserAdmin(userNewDto);
	 * URI uri =
	 * ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand
	 * (user.getId()).toUri(); return ResponseEntity.created(uri).build(); }
	 */
